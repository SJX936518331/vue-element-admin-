    ...
    <div class="user-box">
      <div class="box-center">
        ...
        <div class="cont fl">
          ...
          <div class="con">
            <div
              style="display: inline-block;text-align: left;width: 20%;margin-right: 0%;margin-right: 0%;"
            >
...
import UploadList from "@/components/UploadListItem/uploadList";
import $ from "jquery";
export default {
  name: "user",
  data() {
    ...
    return {
      ...
      // img_src: "src/assets/addImg.png",
      advertiseName: "最新轮播图活动",
      outerChain: "www.xxx.com",
      order: "1",
      submitTxt: "提交",
      outChainTxt: "外链",
      h5Txt: "H5内容",
  ...
  methods: {
    ...
    updateData() {
      ...
      this.$http
        .post(
          "" + this.URL.url + "/api/admin/adbanner/update",
          "adName=" +
            this.advertiseName +
            "&connection=" +
            this.outerChain +
        ...
        .then(
          response => {
            ...
            if (response.data.errmsg == null) {
              this.$notify.success("更新轮播图广告成功");
              console.log("数据", response.data.data);
              this.turn();
            } else {
              this.$Message.error("更新轮播图广告失败");
            }
          },
          response => {
    ...
    turn() {
      this.$router.push("/marketingManagement/rotaryGraphicsA");
    },
    getData() {
      var id = this.$route.params.id;
      this.$http
        .get("" + this.URL.url + "/api/admin/adbanner/edit?id=" + id + "")
        .then(
          response => {
            console.log("请求成功", response.data);
            if (response.data.errmsg == null) {
              ...
              console.log("图片路径值", this.baseForm.picture);
