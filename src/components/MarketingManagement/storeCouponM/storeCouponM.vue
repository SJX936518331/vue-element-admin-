...
export default {
  ...
  methods: {
    ...
    chooseData() {
      this.$http
        .get(
          "" +
            this.URL.url +
            "/api/admin/shopcoupon/list?shopName=" +
            ...
            if (response.data.errmsg == null) {
              ...
              for (var i = 0; i < dataArr.length; i++) {
                this.custList.push({
                  ...
                  decrease:
                    "满" + dataArr[i].enable + "减" + dataArr[i].decrease,
    ...
    removeData() {
      ...
      this.$http
        .post(
          "" + this.URL.url + "/api/admin/shopcoupon/delete",
          "ids=" + ids + ""
        )
        .then(
          response => {
    ...
    refreshData() {
      this.$http
        .get(
          "" +
            this.URL.url +
            "/api/admin/shopcoupon/list?pageNum=" +
            this.currentPage +
            "&pageSize=20"
        )
        .then(
          response => {
            ...
            for (var i = 0; i < dataArr.length; i++) {
              this.custList.push({
                ...
                couponName: dataArr[i].couponName,
                amount: dataArr[i].amount,
                decrease: "满" + dataArr[i].enable + "减" + dataArr[i].decrease,
                promotion: dataArr[i].promotion,
                startDate: dataArr[i].startDate,
                endDate: dataArr[i].endDate,
                limited: dataArr[i].limited,
