...
import UploadList from "@/components/UploadListItem/uploadList2";
export default {
  name: "user",
  data() {
  ...
  methods: {
    ...
    updateData() {
      ...
      this.$http
        .post(
          "" + this.URL.url + "/api/admin/adposition/update",
          "adName=" +
            this.advertiseName +
            "&connection=" +
            this.outerChain +
    ...
    getData() {
      var id = this.$route.params.id;
      this.$http
        .get("" + this.URL.url + "/api/admin/adposition/edit?id=" + id + "")
        .then(
          response => {
            console.log("请求成功", response.data);
            if (response.data.errmsg == null) {
              ...
              if (response.data.data.shape == "平行") {
                this.shapeChoose = 0;
              } else if (response.data.data.shape == "垂直") {
                this.shapeChoose = 1;
              }
            }
          },
