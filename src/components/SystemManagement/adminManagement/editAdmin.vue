        ...
        <div class="cont fl">
          ...
          <div class="con">
            ...
            <el-checkbox
              ...
              :key="index"
              v-model="role.check"
              @change="getValue(role.check,role.id)"
          ...
          <div class="con">
            ...
            <div style="display: inline-block;text-align: left;width: 60%;">
              <v-toggle v-model="toggleTxt" text @change="chageStatus"></v-toggle>
            </div>
          </div>
          <div
            class="btn fl"
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      this.$http.get("" + this.URL.url + "/api/admin/info?id=" + id + "").then(
        response => {
          console.log("请求成功", response.data);
          if (response.data.errmsg == null) {
            this.$Message.success("查看管理员信息成功");
            console.log("数据", response.data.data);
            console.log("数据", id);
            this.id = response.data.data.id;
            this.userName = response.data.data.username;
            this.name_ = response.data.data.name;
            this.department = response.data.data.department;
            this.email = response.data.data.email;
            this.loginFailureCount = response.data.data.loginFailureCount;
            this.toggleTxt = response.data.data.isEnabled;
            this.isLocked = response.data.data.isLocked;
            for (var i = 0; i < this.roles.length; i++) {
              if (this.roles[i].name == response.data.data.roles[i].name) {
        ...
        .then(
          response => {
            ...
            if (response.data.errmsg == null) {
              ...
              this.$router.push({ path: "/systemManagement/adminManagement" });
            }
          },
          response => {
            this.$Message.error("系统出错");
