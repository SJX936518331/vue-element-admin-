    ...
    <div class="user-box">
      ...
      <div class="box-center fl">
        ...
        <div class="cont">
          ...
          >
            <div class="btn fl" style="background: red;" @click="removeData">{{removeTxt}}</div>
            ...
            <el-form-item class="fl" prop="phone">
              <div class="input fl" style="padding: 2px 0px 2px 2px;">
                <span>{{phoneTxt}}</span>
          ...
          >
            ...
            <el-table-column label="商家电话" width="200" align="center">
              <template slot-scope="scope">
                <div
                  v-for="(item,index) in scope.row.shopTemplateDtoList"
                  :key="index"
                >{{item.phone}}</div>
              </template>
            </el-table-column>
            <el-table-column label="店铺" width="150" align="center">
              <template slot-scope="scope">
                <div
                  v-for="(item,index) in scope.row.shopTemplateDtoList"
                  :key="index"
            ...
            <el-table-column label="配送模板名称" width="150" align="center">
              <template slot-scope="scope">
                <div v-for="(items,index) in scope.row.shopTemplateDtoList" :key="index">
                  <div v-for="(item,index) in items.deliveryTemplateList" :key="index">{{item.name}}</div>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="150" align="center">
              <template slot-scope="scope">
                ...
                <div v-for="(items,index) in scope.row.shopTemplateDtoList" :key="index">
                  <div v-for="(item,index) in items.deliveryTemplateList" :key="index">
                    <img
                      style="cursor: pointer;"
                      src="@/assets/search.png"
                      alt
                      @click="edit_redirc(scope.$index,item.id,items.phone,items.shopName)"
                    >
                  </div>
                </div>
              </template>
...
export default {
  ...
  methods: {
    chooseData() {
      this.$http
        .get(
          "" +
            this.URL.url +
            "/api/admin/deliveryTemplate/list?userName=" +
            this.formItem.store +
            "&phone=" +
            this.formItem.phone +
            ""
            ...
            if (response.data.errmsg == null) {
              ...
              for (var i = 0; i < dataArr.length; i++) {
                this.custList.push({
                  ...
                  shopTemplateDtoList: dataArr[i].shopTemplateDtoList
                });
              }
    ...
    refreshData() {
      this.$http
        .get(
          "" +
            this.URL.url +
            "/api/admin/deliveryTemplate/list?pageNum=" +
            this.currentPage +
            "&pageSize=20"
        )
        .then(
    ...
    removeData() {
      ...
      this.$http
        .post(
          "" + this.URL.url + "/api/admin/deliveryTemplate/delete",
          "ids=" + ids + ""
        )
        .then(
          response => {
    ...
    edit_redirc(index, id, phone, shopName) {
      this.$router.push({
