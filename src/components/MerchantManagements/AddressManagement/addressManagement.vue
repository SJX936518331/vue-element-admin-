          ...
          >
            ...
            <el-table-column label="商家电话" width="200" align="center">
              <template slot-scope="scope">
                <div
                  v-for="(item,index) in scope.row.shopReturnAddressDtoList"
                  :key="index"
                >{{item.phone}}</div>
              </template>
            </el-table-column>
            <el-table-column label="店铺" width="200" align="center">
              <template slot-scope="scope">
                <div
                  v-for="(item,index) in scope.row.shopReturnAddressDtoList"
                  :key="index"
            ...
            <el-table-column label="地址" width="150" align="center">
              <template slot-scope="scope">
                <div v-for="(items,index) in scope.row.shopReturnAddressDtoList" :key="index">
                  <div
                    v-for="(item,index) in items.merchantReturnAddressList"
                    :key="index"
                  >{{item.returnAddress}}</div>
                </div>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="150" align="center">
              <template slot-scope="scope">
                <div v-for="(item,index) in scope.row.shopReturnAddressDtoList" :key="index">
                  <img
    ...
    chooseData() {
      this.$http
        .get(
          "" +
            this.URL.url +
            "/api/admin/merchantreturnaddress/list?userName=" +
            this.formItem.store +
            "&phone=" +
            this.formItem.phone +
            ""
            ...
            if (response.data.errmsg == null) {
              ...
              for (var i = 0; i < dataArr.length; i++) {
                this.custList.push({
                  ...
                  shopReturnAddressDtoList: dataArr[i].shopReturnAddressDtoList
                });
              }
    ...
    refreshData() {
      this.$http
        .get(
          "" +
            this.URL.url +
            "/api/admin/merchantreturnaddress/list?pageNum=" +
            this.currentPage +
            "&pageSize=20"
        )
        .then(
