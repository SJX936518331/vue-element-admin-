            ...
            <div class="t1">
              ...
              <div class="table">
                <el-table :data="list1" border style="width: 100%" ref="table">
                  ...
                  <el-table-column prop="defaultAddress" label="默认收件地址" width="300" align="center"></el-table-column>
                </el-table>
              </div>
            </div>
            <div class="t2">
...
export default {
  ...
  methods: {
    turn() {
      this.$router.push("/user/userShopCollection");
    ...
    request() {
      ...
      this.$http
        .get(
          "" +
            this.URL.url +
            "/api/admin/favoriteShop/edit?memberId=" +
            memberId +
            ""
        )
        .then(
          response => {
            ...
            var isVip = response.data.data.isVip == true ? "是" : "否";
            var area =
            ...
            var dataArr = response.data.data.favoriteShops;
            for (var i = 0; i < dataArr.length; i++) {
              this.list2.push({ shopName: dataArr[i].shopName });
            }
          },
          response => {
