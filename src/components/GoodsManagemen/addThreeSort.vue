...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      let firstName = this.$route.params.firstName;
      this.oneSortVal = firstName;
      this.twoSortVal = name;
    },
    addData() {
      let parentId = this.$route.params.parentId;
      let commission = this.$route.params.commission;
      let isShow = this.$route.params.isShow;
      this.$http
        .post(
        ...
        .then(
          response => {
            if (response.data.errmsg == null) {
              this.$notify.success("添加三级分类成功");
              this.$router.push({ path: "/goodsManage/goodsSort" });
            } else {
              this.$Message.error("添加三级分类失败");
            }
