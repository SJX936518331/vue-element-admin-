      ...
      <div class="box-center">
        ...
        <div class="cont">
          ...
          >
            ...
            <el-form-item class="fl" prop="sortName">
              <div class="input fl">
                <span>{{sortName}}</span>
                <el-input
                  style="width:70%;"
        ...
        <div style="position: relative;" class="table">
          ...
          >
              ...
              <template slot-scope="scope">
                <div v-for="(item,index) in scope.row.categoryLv2" :key="index">
                  ...
                  <img
                    ...
                    src="@/assets/add.png"
                    alt
                    @click="turn_one_3(item.id,item.name,scope.row.name,scope.$index)"
                  >
                  <img
                    style="cursor: pointer;"
                    src="@/assets/edit.png"
            ...
            <el-table-column label="三级分类" width="310" align="center">
              <template slot-scope="scope">
                <div v-for="(items,index) in scope.row.categoryLv2" :key="index">
                  <div v-for="(item,index) in items.categoryLv3" :key="index">
                    <span class="sortName">{{item.name}}</span>
                    <img
                      style="cursor: pointer;"
                      src="@/assets/edit.png"
                      alt
                      @click="edit_redirc_3(item.id,items.name,scope.row.name,item.name,items.id,scope.$index)"
                    >
                    <img
                      style="cursor: pointer;"
                      src="@/assets/remove.png"
            ...
            <el-table-column label="佣金比例" width="150" align="center" prop="commission"></el-table-column>
          </el-table>
        </div>
        <!-- <div class="foot">
          <v-pagination
...
export default {
  ...
  methods: {
    ...
    //       "" +
    //         this.URL.url +
    //         "/api/admin/productCategory/list?userName=" +
    //         this.formItem.store +
    //         "&phone=" +
    //         this.formItem.phone +
    //         ""
    ...
    edit_redirc_3(id, name, firstName, cu_name, part_id, index) {
      this.$router.push({
    ...
    refreshData() {
      this.$http
        .get("" + this.URL.url + "/api/admin/productCategory/list")
        .then(
          response => {
            ...
            this.custList = [];
            for (var i = 0; i < dataArr.length; i++) {
              var commission = dataArr[i].commission + "%";
              this.custList.push({
                id: dataArr[i].id,
                name: dataArr[i].name,
                commission: commission,
                image: dataArr[i].image,
                isShow: dataArr[i].show,
                parentId: dataArr[i].parentId,
    ...
    removeData(id) {
      this.$http
        .post(
          "" + this.URL.url + "/api/admin/productCategory/delete",
          "id=" + id + ""
        )
        .then(
          response => {
