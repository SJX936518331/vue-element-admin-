      ...
      <div class="box-center">
        ...
        <div class="cont fl">
          <el-form :model="formItem" ref="formItem" :rules="ruleInline" :label-width="label_width">
            <el-form-item prop="oneSort">
              <div class="input">
                <span>{{oneSort}}</span>
                <span style="font-weight: normal;">{{oneSortVal}}</span>
              </div>
            </el-form-item>
            <el-form-item prop="oneSort">
              <div class="input">
                <span>{{twoSort}}</span>
                <span style="font-weight: normal;">{{twoSortVal}}</span>
              </div>
            </el-form-item>
...
import $ from "jquery";
import UploadList from "@/components/UploadListItem/uploadList5";
export default {
  name: "user",
  data() {
      ...
      this.$http
        ...
        .then(
          response => {
            console.log("请求成功", response.data);
            if (response.data.errmsg == null) {
              this.$Message.success("编辑三级分类成功");
              console.log("数据", response.data.data);
              this.oneSortVal = response.data.data.topCategory.name;
              this.twoSortVal = response.data.data.parentCategory.name;
